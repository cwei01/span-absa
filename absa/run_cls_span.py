...
def metric_max_over_ground_truths_NER(metric_fn,term,polarity, gold_terms,gold_polarities):
...
def eval_absa(all_examples, all_features, all_results, do_lower_case, verbose_logging, logger):
    ...
    for (feature_index, feature) in enumerate(all_features):
        ...
        for term, polarity in zip(pred_terms, pred_polarities):
            common += metric_max_over_ground_truths_NER(exact_match_score, term, polarity, example.term_texts, example.polarities)
