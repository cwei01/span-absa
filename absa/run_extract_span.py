...
def read_train_data(args, tokenizer, logger):
    ...
    train_dataloader = DataLoaderX(train_data, sampler=train_sampler, batch_size=args.train_batch_size)
...
def run_train_epoch(args, global_step, model, param_optimizer, train_dataloader,
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        loss = model(input_ids, segment_ids, input_mask, start_positions, end_positions,args.weight_start,args.weight_end,args.weight_span,args.weight_bias)
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % save_checkpoints_steps == 0 and count != 0:  # eval & save model
...
def evaluate(args, model, device, eval_examples, eval_features, eval_dataloader, logger, write_pred=False, do_pipeline=False):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            batch_start_logits, batch_end_logits , batch_span_logits= model(input_ids, segment_ids, input_mask)
...
def main():
    ...
    parser.add_argument("--logit_threshold", default=None, type=float,required=True)
    ...
    parser.add_argument("--weight_bias", default=1500., type=float)
    #parser.add_argument("--train_file", default=os.path.join("split_twitter%s_train.txt" %(i)), type=str)
    #parser.add_argument("--predict_file", default=os.path.join("twitter%s_test.txt" %(i)) , type=str)
