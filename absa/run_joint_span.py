...
def run_train_epoch(args, global_step, model, param_optimizer,
    ...
    y=len(train_dataloader) #270
    for step, batch in enumerate(train_dataloader):
        if n_gpu == 1:
            batch = tuple(t.to(device) for t in batch)  # multi-gpu does scattering it-self
        input_ids, input_mask, segment_ids, start_positions, end_positions, example_indices = batch
        batch_start_logits, batch_end_logits, batch_span_logits, _ = model('extract_inference', input_mask, input_ids=input_ids, token_type_ids=segment_ids,
                                                                           layer_GRU=args.layer_GRU)

        batch_features, batch_results ,batch_logits= [], [], []
        for j, example_index in enumerate(example_indices):
            ...
            span_logits = batch_span_logits[j].detach().cpu().tolist()
            ...
            batch_logits.append(span_logits)
        span_starts, span_ends, labels, label_masks = span_annotate_candidates(train_examples, batch_features,
                                                                               ...
                                                                               args.do_lower_case,
                                                                               args.verbose_logging, logger,batch_logits)
        ...
        if random_number < args.random_train:
            loss = model('train', input_mask, input_ids=input_ids, token_type_ids=segment_ids,
                         ...
                         polarity_labels=labels, label_masks=label_masks,layer_GRU=args.layer_GRU,weight_start=args.weight_start,weight_end=args.weight_end,
                                                                               weight_span=args.weight_span,weight_ac=args.weight_ac,use_expectation=False  #use ground_truth
                         ,)
        else:
            loss = model('train', input_mask, input_ids=input_ids, token_type_ids=segment_ids,
                         start_positions=start_positions, end_positions=end_positions,
                         span_starts=span_starts, span_ends=span_ends,
                         polarity_labels=labels, label_masks=label_masks, layer_GRU=args.layer_GRU,
                         weight_start=args.weight_start, weight_end=args.weight_end,
                         weight_span=args.weight_span, weight_ac=args.weight_ac, use_expectation=True, ) #expection
...
def evaluate(args, model, device, eval_examples, eval_features, eval_dataloader, logger, write_pred=False):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            batch_start_logits, batch_end_logits, batch_span_logits,sequence_output = model('extract_inference', input_mask,
                                                                          input_ids=input_ids,
                                                                          token_type_ids=segment_ids,layer_GRU=args.layer_GRU)
...
def main():
    ...
    parser.add_argument("--logit_threshold", default=None, type=float,required=True)  # res_9.6
    parser.add_argument("--random_train", default=None, type=float,required=True)
    parser.add_argument("--weight_span", default=None, type=float,required=True)
    parser.add_argument("--weight_ac", default=None, type=float,required=True)
    ...
    if args.do_predict:
        ...
        print("start delete the model file.......")
        if os.path.exists(save_path):
            os.remove(save_path)
            print("the model file has been deleted !")
        else:
            print("no such file")
