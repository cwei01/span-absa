...
def span_annotate_candidates(all_examples, batch_features, batch_results, filter_type, is_training, use_heuristics, use_nms,
                             logit_threshold, n_best_size, max_answer_length, do_lower_case, verbose_logging, logger,batch_logits):
    ...
    for (feature_index, feature) in enumerate(batch_features):
        ...
        if is_training:
            ...
            # max_score_start = 0
            # k=end_indexes[0]
            # start_logit = result.start_logits[start_indexes[0]]
            # end_logit = result.end_logits[end_indexes[0]]
            # for end_index in end_indexes:
            #     if start_indexes[0] >= len(feature.tokens):
            #         continue
            #     if end_index >= len(feature.tokens):
            #         continue
            #     if start_indexes[0] not in feature.token_to_orig_map:
            #         continue
            #     if end_index not in feature.token_to_orig_map:
            #         continue
            #     if end_index < start_indexes[0]:
            #         continue
            #     length = end_index - start_indexes[0] + 1
            #     if length > max_answer_length:
            #         continue
            #     if batch_logits[feature_index][start_indexes[0]][end_index]>max_score_start:
            #         max_score_start=batch_logits[feature_index][start_indexes[0]][end_index]
            #         k=end_index
            #         start_logit = result.start_logits[start_indexes[0]]
            #         end_logit = result.end_logits[k]
            #     # if batch_logits[feature_index][start_indexes[0]][end_index]<logit_threshold:
            #     #     continue
            ...
            for start_index in start_indexes:
                for end_index in end_indexes:
                    ...
                    if length > max_answer_length:
                        continue
                    if batch_logits[feature_index][start_index][end_index] < 1e-2:
                        continue
            ...
            if use_heuristics:
                prelim_predictions_per_feature = sorted(
                    prelim_predictions_per_feature,
                    key=lambda x: (start_logit+end_logit-(x.end_index - x.start_index + 1)),
                    reverse=True)
            else:
                prelim_predictions_per_feature = sorted(
                    prelim_predictions_per_feature,
